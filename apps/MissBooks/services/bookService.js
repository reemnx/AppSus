...
function query(filterBy) {
    if (!filterBy) var books = gBooks;
    else {
        var { title, maxPrice, minPrice } = filterBy
        maxPrice = maxPrice ? maxPrice : Infinity
        minPrice = minPrice ? minPrice : 0
        var books = gBooks.filter(book => book.title.includes(title)
...
function saveReview(id, review) {
    let idx = gBooks.findIndex(book => book.id === id);
    if (!gBooks[idx].reviews) gBooks[idx].reviews = [];
    gBooks[idx].reviews.unshift(review);
    bookStorage.saveToStorage(KEY, gBooks);
}
...
function removeReview(bookId, reviewId) {
    ...
    gBooks[idx].reviews.splice(reviewId, 1);
    bookStorage.saveToStorage(KEY, gBooks);
}

function getGoogleBooksByKey(searchKey) {
    let googleBooks;
    return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchKey}`)
        .then(res => res.data.items)
        .then(res => {
            googleBooks = res.map(book => {
                return {
                    id: book.id,
                    title: book.volumeInfo.title,
                    authors: book.authors,
                    publishedDate: book.publishedDate,
                    description: book.volumeInfo.description ? book.volumeInfo.description : "Lorem ipsum dolor sit amet consectetur adipisicing elit. Sequi eaque sapiente autem blanditiis, repellendus quaerat cupiditate sed repellat pariatur sunt accusantium, consequuntur consequatur perspiciatis vero eius. Adipisci ut beatae ratione fugiat fuga earum quae quam ullam consectetur repudiandae obcaecati mollitia ad, incidunt, autem aperiam error quasi. Debitis error consequuntur unde, rerum tempore nihil impedit obcaecati iusto qui quos possimus nulla omnis reiciendis minus aliquid pariatur est aspernatur? Quod culpa molestias aliquid ex saepe eum consequuntur eligendi consequatur laborum",
                    pageCount: book.volumeInfo.pageCount,
                    categories: book.volumeInfo.categories,
                    thumbnail: book.volumeInfo.imageLinks ? book.volumeInfo.imageLinks.thumbnail : "http://books.google.com/books/content?id=BeCBnxPR8NcC&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api",
...
function addGoogleBook(bookToAdd) {
    const existsBook = gBooks.find(book => book.id === bookToAdd.id)
    if (!existsBook) {
...
function getPrevNextBooks(id) {
    let idx = gBooks.findIndex(book => book.id === id);
    let prev = (idx !== 0) ? gBooks[idx - 1].id : gBooks[gBooks.length - 1].id;
    let next = (idx !== (gBooks.length - 1)) ? gBooks[idx + 1].id : gBooks[0].id;
    return {
        prev,
        next
    }
...
var gBooks = bookStorage.loadFromStorage(KEY) || [
    {
        "id": "OXeMG8wNskc",
